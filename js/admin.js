...
function renderDocsChecklist(){
  qs('docPermissions').innerHTML = state.members.map(m => `<label class="checkitem"><input type="checkbox" value="${m.id}"> <span>${esc(m.name)} • ${esc(m.role)}</span></label>`).join('');
}
...
function memberLink(memberId){
  ...
  return url.href.replace(/index_admin\.html.*$/, `membro.html?member=${encodeURIComponent(memberId)}`);
}
function renderQr(){
...
function render(){
  const unitsMap = mapById(state.units), membersMap = mapById(state.members), sessionsMap = mapById(state.sessions);
  ...
  qs('logoLeftPath').value = state.config.logos?.left || '';
  qs('logoRightPath').value = state.config.logos?.right || '';
...
window.addEventListener('load', () => {
  ...
  qs('saveUnit').onclick = () => {
    ...
    set(push(unitsRef), payload);
  };
  qs('saveMember').onclick = () => {
    if(!qs('memberName').value.trim() || !qs('memberRole').value.trim()) return alert('Preencha nome e função');
    set(push(membersRef), {name:qs('memberName').value.trim(), role:qs('memberRole').value.trim()});
  };
  qs('saveSession').onclick = () => {
    if(!qs('sessionName').value.trim() || !qs('sessionStart').value || !qs('sessionEnd').value) return alert('Preencha a sessão');
    set(push(sessionsRef), {name:qs('sessionName').value.trim(), start:qs('sessionStart').value, end:qs('sessionEnd').value});
  };
  qs('saveTask').onclick = () => {
    ...
    set(push(tasksRef), { title:qs('taskTitle').value.trim(), description:qs('taskDesc').value.trim(), ownerIds, unitId:qs('taskUnit').value, sessionId:qs('taskSession').value, due:qs('taskDue').value, status:'pendente', type:'individual', lastUpdatedBy:'' });
  };
  qs('saveDoc').onclick = () => {
    const name = qs('docName').value.trim(), url = qs('docPath').value.trim(), allowedMemberIds = getSelectedDocPermissions();
    if(!name || !url) return alert('Preencha nome e caminho do PDF');
    set(push(docsRef), {name, url, allowedMemberIds});
  };
});
