...
function memberLink(memberId){
  const base = `${window.location.origin}/cronograma_stockcar/`;
...
window.addEventListener('load', () => {
  ...
  qs('saveUnit').onclick = () => {
    ...
    if(type === 'car'){ if(!qs('unitNumber').value.trim() || !qs('unitDriver').value.trim()) return alert('Preencha carro e piloto'); payload.number = qs('unitNumber').value.trim(); payload.driver = qs('unitDriver').value.trim(); }
    else { if(!qs('unitName').value.trim()) return alert('Preencha o nome da coluna'); payload.name = qs('unitName').value.trim(); }
