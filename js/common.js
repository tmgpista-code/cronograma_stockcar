...
export const rootRef = ref(db, "stockcar");
...
export function countdown(target,status,mode){
  if(status==='concluida') return {text:'CONCLUÍDA',cls:'gray'};
  const diff=new Date(target)-new Date();
  const late=diff<0, abs=Math.abs(diff);
  const h=Math.floor(abs/3600000), m=Math.floor(abs%3600000/60000), s=Math.floor(abs%60000/1000);
  if(mode==='session' && late) return {text:'INICIADO',cls:'red'};
  const text=(late?'ATRASADO ':'')+[h,m,s].map(v=>String(v).padStart(2,'0')).join(':');
  const cls=late?'red':diff<=300000?'amber':'green';
  return {text,cls};
}
